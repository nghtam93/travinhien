...
function breadcrumbs_woo(){
  if(is_cart()||is_checkout()){
    ...
    ?>
    <nav class="breadcrumbs breadcrumbs__divider">
      <div class="container">
        <div class="d-flex align-items-center justify-content-center">
          <a href="<?php echo wc_get_cart_url(); ?>" class="<?php echo $class1; ?>">
            <span class="breadcrumb-step d-none d-md-inline-block">1</span>
            <?php _e( 'Giỏ hàng', 'dntheme' ); ?>
          </a>
          <span class="divider"><i class="icon-arrow-right d-none d-sm-inline-block"></i></span>
          <a href="<?php echo wc_get_checkout_url() ; ?>" class="<?php echo $class2; ?>">
            <span class="breadcrumb-step d-none d-md-inline-block">2</span>
            <?php _e( 'Đặt hàng', 'dntheme' ); ?></a>
          <span class="divider"><i class="icon-arrow-right d-none d-sm-inline-block"></i></span>
          <a href="#" class="<?php echo $class3; ?> no-click">
            <span class="breadcrumb-step d-none d-md-inline-block">3</span>
            <?php _e( 'Hoàn thành', 'dntheme' ); ?>
...
function customizing_woocommerce_description( $content ) {
    ...
    if ( is_product() ) {
      global $product;
        ...
        // The custom content
        $custom_content = 'Fanpage: '.get_field('facebook','option').'<br>';
        $custom_content .= 'Hotline: '.get_field('phone','option').'<br>';
        $custom_content .= __("Khối lượng tịnh:","dntheme").$product->get_weight() .$weight_unit;
...
add_filter( 'woocommerce_localisation_address_formats', 'woocommerce_custom_address_format', 999999 );
function woocommerce_custom_address_format( $formats ) {
    $formats[ 'default' ]  = "{name}, {address_1}, {address_2}, {city}, {state_code} {country}";
